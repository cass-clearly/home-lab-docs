...
- Final verification: after the network repair, `TV 2021` at `192.168.4.22` successfully fetched library pages, metadata, artwork, notifications, and the companion-player poll directly from Plex, but pressing Play still generated **no media-decision or media-file request**. A separate iPhone on the same LAN immediately direct-played the same server content. Therefore a persistent all-title Playback Error on this TV is now conclusively a Samsung TV firmware / OS / app-runtime problem, not Plex, the network, media files, or storage. Next recovery path: cold-boot the TV (hold remote Power until the Samsung logo), update TV firmware, then reset the TV network connection if necessary.
